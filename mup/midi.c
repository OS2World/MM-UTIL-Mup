...
/* Copyright (c) 1995, 1996, 1997, 1998, 1999, 2000, 2001, 2002, 2004, 2005,
 * 2006, 2007, 2009, 2010 by Arkkra Enterprises */
/* All rights reserved */
...

{
	...
	if ((*mll_p_p)->u.bar_p->endingloc == STARTITEM
		...
		for (*mll_p_p = (*mll_p_p)->next;
			...
			/* We need to honor visibility changes
			 * (actually audibility for midi purposes), since
			 * something that became visible during the first
			 * ending would still be visible for the second
			 * ending unless explicitly made invisible, and
			 * user would probably expect audibility to act
			 * the same way. But we don't want other SSV things
			 * in the first ending to apply now. E.g., if the
			 * key or time changed during the first ending,
			 * we should ignore that while skipping to the second
			 * ending. So we only apply visibility changes. */
			else if ((*mll_p_p)->str == S_SSV &&
					(*mll_p_p)->u.ssv_p->used[VISIBLE]
					== YES) {
				struct SSV *tempssv_p;
				CALLOC(SSV, tempssv_p, 1);
				tempssv_p->context = (*mll_p_p)->u.ssv_p->context;
				tempssv_p->staffno = (*mll_p_p)->u.ssv_p->staffno;
				tempssv_p->voiceno = (*mll_p_p)->u.ssv_p->voiceno;
				tempssv_p->used[VISIBLE] = YES;
				tempssv_p->visible = (*mll_p_p)->u.ssv_p->visible;
				asgnssv(tempssv_p);
				FREE(tempssv_p);
			}
		}
	...
	for (  ; gs_p != (struct GRPSYL *) 0; gs_p = gs_p->next) {
		...
		for (n = 0; n < gs_p->nnotes; n++) {
			...
			if ( (gs_p->notelist[n].tie == YES) ||
						(gs_p->tie == YES) ) {
				if (gs_p->notelist[n].tied_to_voice
							== NO_TO_VOICE) {
					Tie_table[raw_notenum] = YES;
				}
				else {
					/* It would be nice if we could
					 * do the tie to tied-to voice.
					 * But if the voice in on a different
					 * channel, then it is impossible.
					 * Even if it is the same channel,
					 * we put every voice in a separate
					 * track, so it is not entirely
					 * clear if all MIDI players would
					 * handle a note being turned on in
					 * one track and off in a different
					 * one. Even if that would work,
					 * we process each track in a
					 * separate pass through the main
					 * list, so while it would be easy
					 * enough to ignore the normal
					 * "note off" in the current voice,
					 * identifying that the other voice
					 * should not have a "note on"
					 * would be harder. Probably the best
					 * way would be to have parse set a
					 * new tied_from_voice field in NOTE,
					 * so our MIDI code earlier
				 	 * in this function could treat that
					 * field being other than NO_TO_VOICE
					 * like Tie_table[raw_notenum]
					 * being YES. But only if the channels
					 * match, which we wouldn't know
					 * without considerable work.
					 * Basically, we would have to walk
					 * through the tied_from_voice's
					 * GRPSYLs, either from the beginning
					 * to current place, or backwards
					 * from current place til a channel
					 * change STUFF is found.
					 * That seems too much work to bother
					 * for such a fringe case, especially
					 * since we can never do non-channel-
					 * matched cases, which may seem the
					 * same to a user, so may be better
					 * to consistently not even try.
					 */
				}
			}
			else {
...

{
	...
	*raw_notenum_p = val;
	...
	if (note_p->accidental == '\0') {
		/* leave accidental as is from key signature */
		acc_adjust = Accidental_map[val];
	}
	else {
		acc_adjust = strchr(Acclets, note_p->accidental) - Acclets - 2;
	}
