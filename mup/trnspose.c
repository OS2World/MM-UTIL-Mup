/* Copyright (c) 1995, 1997, 1998, 1999, 2000, 2002, 2004, 2005, 2007
...
static void fixslurto P((int s, struct MAINLL *mainll_p, int nintnum,
		int noctint));
static void apply_useaccs P((void));
...

{
	...
	char pitchacc;		/* acc for this pitch, regardless of whether we
				 * are actually going to print it */
	...
	for (;;) {
		...
		for (n = 0; n < g_p->nnotes; n++) {
			...
			curacc_p = &curacc[ note_p->letter - 'a' ]
					  [ note_p->octave ];
			/*
			 * If the note already has an accidental (supplied by
			 * the user), remember it (if we should) and don't
			 * change it.
			 */
			if (note_p->accidental != '\0') {
				if (carryaccs == YES) {
					*curacc_p = note_p->accidental;
				}
				continue;
			}
			...
			if (*curacc_p != '\0') {
				/* an acc is in effect; that is the pitch */
				pitchacc = *curacc_p;
			} else {
			 ...
			 */
			if (g_p->grpvalue == GV_NORMAL) {
				allow_acc = YES;
				for (g2_p = stafftime2firstgrp(
						mll_p->u.staff_p, mintime);
			...
			case UA_Y_NONE:
				 ...
				 */
				if (pitchacc != *curacc_p &&
				   ! (pitchacc == 'n' && *curacc_p == '\0')) {
					note_p->accidental = pitchacc;
					if (carryaccs == YES) {
						*curacc_p = pitchacc;
					}
				}
...

{
	...
	for (n = 0; n < g_p->nnotes; n++) {
		if (g_p->notelist[n].letter == note_p->letter &&
			...
			if (g_p->notelist[n].accidental != '\0') {
				return (YES);
			} else {
				return (NO);
			}
