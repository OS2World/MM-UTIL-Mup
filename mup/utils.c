 ...
 * 2006, 2007, 2008, 2009, 2010, 2011 by Arkkra Enterprises */
/* All rights reserved */
...

{
	...
	if (gs_p->grpcont == GC_REST) {
		if (gs_p->stemdir == UP) {
			return(gs_p->c[AN] - withheight(gs_p));
		}
		else {
			return(gs_p->c[AS] + withheight(gs_p));
		}
	}
...

{
	...
	if (mainll_p->u.feed_p->rightmargin == MG_AUTO) {
		/*
		...
		if (mll_p->next->str == S_BLOCKHEAD) {
			/* blow away the useless MG_AUTO */
			mainll_p->u.feed_p->rightmargin = MG_DEFAULT;
			return (Score.rightmargin / Score.scale_factor);
		}

		/* not after a block: return this flag value */
		return (MG_AUTO);
	}

	/* if there is no override, use the parameter */
	if (mainll_p->u.feed_p->rightmargin < 0.0)
		return (Score.rightmargin / Score.scale_factor);
