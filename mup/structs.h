 ...
 * 2005, 2006, 2007, 2008, 2009, 2010 by Arkkra Enterprises */
/* All rights reserved */
/*
 *	structs.h
 *
...
struct STUFF {
	...
	float horzscale;	/* horz scaling of an aligned STUFF */
...
struct NOTE {
	...
	unsigned tiestyle	: 3;
	unsigned tiedir		: 2;
	unsigned tied_to_voice	: 2;
	unsigned tied_from_other: 1;
	unsigned slurred_from_other: 1;
...
struct GRPSYL {
	 ...
	 * stemlen_forced tells whether the user forced stemlen's value.
	 * These fields are valid only for note groups.
	 */
	float stemlen;
	short stemdir;		/* up or down */
	float stemx;
	short stemlen_forced;
	 ...
	 * of the group, but in some cases not; setwithside() sets normwith
	 * to say YES in the normal case, otherwise NO.  These fields are not
	 * valid for space groups.
	 */
	...
	unsigned beamloc	: 3;
	unsigned stemdir	: 2;
	unsigned stemlen_forced	: 1;
	unsigned tupside	: 2;
	unsigned phraseside	: 2;
	unsigned phcount	: PH_BITS;
	unsigned ephcount	: PH_BITS;
	unsigned printtup	: 2;
	unsigned tie		: 1;
	unsigned normwith	: 1;
	unsigned roll		: 3;
	...
	unsigned sep_accs	: 1;
	unsigned with_was_gtc	: 1;
	unsigned padded_csb_stem: 1;
	unsigned alignrests	: 1;
	unsigned clef_vert	: 1;
#endif
};
