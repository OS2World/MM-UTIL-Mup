 ...
 * 2005, 2007, 2008, 2010 by Arkkra Enterprises */
/* All rights reserved */
/*
 * Name:	beamstem.c
 *
...

{
	...
	float bf;		/* number of beams/flags */
	float accbf;		/* special use for accidental situation */
	...
	for (gs_p = start_p; gs_p != end_p; gs_p = nextsimilar(gs_p)) {
		...
		if (gs_p != start_p && gs_p->stemdir == UP &&
				...
				b1 > 0 && bf > 1) {
			bf += 3.5 * b1 * (Stepsize / Flagsep) * ((accbf > 1) +
...

{
	...
	float *yg_p, *ng_p, *sg_p;	/* point at group coordinates */
...

{
	...
	num = tupgroups(start_p, &numng, &numnrg, &startng_p, &lastng_p,
...
{
	float hi;		/* height of the "with" list */
	...
	/*
	 * Get the height of the "with" items, and move the group's N or S
	 * coordinate to contain them.
	 */
	hi = withheight(gs_p);
	if ((gs_p->stemdir == DOWN) == (gs_p->normwith == YES)) {
		gs_p->c[RN] += hi;
	} else {
		gs_p->c[RS] -= hi;
	}
