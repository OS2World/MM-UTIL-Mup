 ...
 * 2007, 2009, 2010  by Arkkra Enterprises */
/* All rights reserved */
/*
 * Name:	abshorz.c
 *
...

{
	...
	for (;;) {
		...
		for (s = 1; s <= Score.staffs; s++) {
			...
			oldkey[s] = eff_key(s);
			oldnormkey[s] =
				svpath(s, STAFFLINES)->stafflines == 5 &&
		...
		if (strcmp(Score.timerep, oldtimerep) != 0 && Score.timevis == YES)
			timechg = YES;
		else
			timechg = NO;
...

{
	...
	*scores_p = 0;			/* no scores yet */
	...
	for (;;) {
		...
		if (measinscore[*scores_p] < maxmeasures &&
...

{
	...
	struct STUFF *prev_p;		/* the one that preceeds stuff_p */
