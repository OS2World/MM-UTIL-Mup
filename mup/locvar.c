 ...
 * 2008, 2009, 2010, 2011 by Arkkra Enterprises */
/* All rights reserved */
...

{
	...
	for (n = 0; n < curve_p->ncoord - 1; n++) {
		 ...
		 * reach the end of the score.
		 */
		curve_p->coordlist[ncoord1 - 1] = curve_p->coordlist[n];
		offset = PGWIDTH - eff_rightmargin(mll_p)
				- inpc_x( &(curve_p->coordlist[n]),
				fname, lineno );
...

{
	...
	y_offset_of_left_point_from_its_staff =
			getYstaff(left_mll_feed_p, left_staffnum) - left_y;
	y_offset_of_right_point_from_its_staff =
			getYstaff(right_mll_feed_p, right_staffnum) - right_y;
