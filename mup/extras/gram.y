...
assign:	T_NUMVAR T_EQUAL opt_minus num
	...
	T_UNSET paramname
	{
		if (contextcheck(C_STAFF|C_VOICE, "unset") == YES) {
			if ($2 < 0 || $2 >= NUMFLDS) {
				pfatal("invalid parameter index %d", $2);
			}
	...
	|
	T_TRANSPOSE T_EQUAL updown transpose_interval num
	{
		char *trans_name;
...
stafflinedef:
	...
	|
	T_TAB T_LPAREN tab_string_list T_RPAREN
	{
		/* user-specified tablature */
		asgn_stafflines($3, NO, Currstruct_p);
	};
...
minscsep:	num
	{
		if (Currstruct_p == 0) {
			 ...
			 * between scores. */
			assign_int(MINSCSEP, $1, Currstruct_p);
		}
	};
...
maxscsep:
	...
	T_COMMA num
	{
		if (Currstruct_p != 0) {
			...
			assign_int(MAXSCSEP, $2, Currstruct_p);
		}
	};

minscpad:	opt_minus num
	{
		if (Currstruct_p == 0) {
			l_warning(Curr_filename, yylineno,
			...
			/* Assign the value of minimum vertical space
			 * between scores. */
			assign_int(MINSCPAD, $1 * $2, Currstruct_p);
		}
	}
	;
...
titleA:
	string 
	{
		Extra = extra_needed(Titlefont, Titlesize, $1);
		proc_printcmd(J_CENTER, (struct INPCOORD *) 0, $1,
	...
	string string
	{
		...
		Extra = extra_needed(Titlefont, Titlesize, $1);
		tmp_extra = extra_needed(Titlefont, Titlesize, $2);
		Extra = MAX(Extra, tmp_extra);


		proc_printcmd(J_LEFT, (struct INPCOORD *) 0, $1,
				Titlefont, Titlesize, YES, NO, Extra);
		proc_printcmd(J_RIGHT, (struct INPCOORD *) 0, $2,
	...
	string string string
	{
		...
		tmp_extra = extra_needed(Titlefont, Titlesize, $2);
		Extra = MAX(Extra, tmp_extra);
		tmp_extra = extra_needed(Titlefont, Titlesize, $3);
		Extra = MAX(Extra, tmp_extra);

		proc_printcmd(J_LEFT, (struct INPCOORD *) 0, $1,
				Titlefont, Titlesize, YES, NO, Extra);
		proc_printcmd(J_CENTER, (struct INPCOORD *) 0, $2,
				Titlefont, Titlesize, NO, NO, (double) 0.0);
		proc_printcmd(J_RIGHT, (struct INPCOORD *) 0, $3,
				Titlefont, Titlesize, NO, NO, (double) 0.0);
	};
...
paragraph:	opt_paratype T_PARAGRAPH tfont title_size string opt_semi T_NEWLINE
	{
		...
		(void) fix_string(string_start, font, size,
					Curr_filename, yylineno);
		proc_printcmd($1, (struct INPCOORD*) 0, string_start,
...
markitem:	T_WITH alloc_marklist marks 
	{
...
notedata1:	pitch opt_bend
	{
		if (Doing_tab_staff == YES) {
			add_note(Curr_grpsyl_p, $1, 0, NOFRET, 0, NO, $2);
		}
		else {
			add_note(Curr_grpsyl_p, $1, 0, USE_DFLT_OCTAVE, 0, NO, $2);
		}
	}

	|
	pitch T_LPAREN acc_symbol T_RPAREN opt_octave
	{
		if (Doing_tab_staff == YES) {
			l_warning(Curr_filename, yylineno,
				"parentheses around accidentals are extraneous on tab staffs; ignoring");
		}
		add_note(Curr_grpsyl_p, $1, $3, $5, 0, YES, (char *) 0);
	}
	...
	pitch T_LPAREN octave T_RPAREN opt_bend
	{
		...
		add_note(Curr_grpsyl_p, $1, 0, $3, 0, YES, $5);
	}
	...
	pitch T_LPAREN pitch
	{
		...
		pushback(yytext[0]);
		pushback('(');
		add_note(Curr_grpsyl_p, $1, 0,
			(Doing_tab_staff ? NOFRET : USE_DFLT_OCTAVE),
			0, NO, (char *) 0);
	}

	|
	pitch acc_symbol opt_ticks opt_octave opt_bend
	{
		add_note(Curr_grpsyl_p, $1, $2, $4, $3, NO, $5);
	}

	|
	pitch acc_symbol opt_ticks T_LPAREN octave T_RPAREN opt_bend
	{
		if (Doing_tab_staff == NO) {
			l_warning(Curr_filename, yylineno,
				"extraneous parentheses around octave; ignoring");
		}
		add_note(Curr_grpsyl_p, $1, $2, $5, $3, YES, $7);
	}

	|
	pitch acc_symbol opt_ticks T_LPAREN pitch
	{
		/* Whoops! The left paren was really for a new note,
		 * which happens to be parenthesized. But beause yacc only
		 * looks ahead one token, it finds out too late. So we
	...
	{
		add_note(Curr_grpsyl_p, $1, 0, $3, $2, NO, $4);
	}

	|
	pitch T_TICKS T_LPAREN octave T_RPAREN opt_bend
	{
		add_note(Curr_grpsyl_p, $1, 0, $4, $2, YES, $6);
	}

	|
	pitch octave opt_accidental opt_bend
	{
		if (Doing_tab_staff == YES && $3 != '\0') {
			yyerror("accidental must be before fret number");
		}
		add_note(Curr_grpsyl_p, $1, $3, $2, 0, NO, $4);
	}

	|
	pitch octave T_LPAREN acc_symbol T_RPAREN
	{
		add_note(Curr_grpsyl_p, $1, $4, $2, 0, YES, (char *) 0);
	}
	...
	pitch octave T_LPAREN pitch
	{
		...
		pushback(yytext[0]);
		pushback('(');
		add_note(Curr_grpsyl_p, $1, 0, $2, 0, NO, (char *) 0);
	};
...
lyricinfo:	T_LYRICS lyric_id T_COLON lyricdata
	{
	};
...
opt_temp_margin:
	...
	|
	opt_temp_margin temp_margin
	;

temp_margin:	T_FNUMVAR opt_equals margin_override
	{
		if ($3 > 0.0) {
		...
		if ($1 == RIGHTMARGIN) {
			...
			Currstruct_p->u.feed_p->rightmargin = $3;
		}
			...
			Currstruct_p->u.feed_p->leftmargin = $3;
		}
		else {
...
barinfo:	linetype precbartype barline
	{
		...
		if (contextcheck(C_MUSIC, "bar") == YES) {
			...
			Currstruct_p = add_bar($3, $1, Endingloc, Mainlltc_p);
			Got_ending = NO;
		}
...
printcmd:	printtype opt_loc string
	{
		...
		Extra = extra_needed(Curr_family + Curr_font, Curr_size, $3);
		proc_printcmd($1, Curr_loc_info_p, $3, Curr_family + Curr_font,
			Curr_size, $2, NO, ($2 == YES ? Extra : (double) 0.0));
	}

	|
	postscript opt_loc string
	{
		if ($2 == YES) {
			warning("nl ignored for postscript");
		}
		proc_printcmd(J_NONE, Curr_loc_info_p, $3, FONT_UNKNOWN,
