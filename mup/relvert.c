 ...
 * 2005, 2007, 2010 by Arkkra Enterprises */
/* All rights reserved */
/*
 * Name:	relvert.c
 *
...
static int do_this_pass P((struct STUFF *stuff_p, int place,
		unsigned long do_which));
...

{
	...
	for (mainll_p = start_p->next; mainll_p != 0 &&
		...
		for (stuff_p = mainll_p->u.staff_p->stuff_p;
			...
			if (do_this_pass(stuff_p, place, do_which) == NO) {
				continue;
			}
...
static int
do_this_pass(stuff_p, place, do_which)
...
#define SDIST		c[AS]
	...
	for (n = nstuffs - 2; n >= 0; n--) {
		...
		/*
		 * If there was no extender, or it was smaller than the
		 * original overlap, we'll still have some overlap.  Shrink
		 * just enough to avoid collision, but no more than
		 * MINHORZSCALE allows.  If it isn't enough, warn.  We go right
		 * to left because the amount of shrinkage needed for a
		 * rectangle depends on the (possibly shrunk) size of the
		 * rectangle to its right.  The theory here is that many STUFFs
		 * stick out much farther to the right of AX than to the left.
		 */
		if (overlap > 0.0) {
			fact = (Rectab[ridx + 1].w - stufflist[n]->c[AX]) /
			       (Rectab[ridx].e     - stufflist[n]->c[AX]);
			if (fact < MINHORZSCALE) {
				fact = MINHORZSCALE;
				l_warning(stufflist[n]->inputfile,
				stufflist[n]->inputlineno,
				"cannot shrink aligned mark enough to avoid collision with the next one");
			}

			/* tell print phase how much to shrink */
			stufflist[n]->horzscale = fact;

			/* revise the boundaries in Rectab */
			Rectab[ridx].e = stufflist[n]->c[AX] + fact *
				(stufflist[n]->c[AE] - stufflist[n]->c[AX]);
			Rectab[ridx].w = stufflist[n]->c[AX] + fact *
				(stufflist[n]->c[AW] - stufflist[n]->c[AX]);

			/* revise the boundaries in the STUFF structure too */
			stufflist[n]->c[AE] = Rectab[ridx].e;
			stufflist[n]->c[AW] = Rectab[ridx].w;
		}
	}
