/* Copyright (c) 1995, 1997, 1998, 1999, 2000, 2001, 2004, 2005, 2006, 2007,
 * 2008, 2009, 2010 by Arkkra Enterprises */
/* All rights reserved */
/*
 * Name:	setgrps.c
 *
		...
		for (gs2_p = gs1_p->gs_p; gs2_p != 0 &&
			...
			for (n = 0; n < gs1_p->nnotes; n++) {
				...
				for (k = 0; k < gs2_p->nnotes; k++) {
					...
					/* skip if it has no acc */
					if (gs2_p->notelist[k].accidental ==
							'\0') {
						continue;
					}

					/* if they contradict, return YES */
					if (gs1_p->notelist[n].accidental !=
					    gs2_p->notelist[k].accidental) {
						return (YES);
					}
				}
			}
...

{
	...
	for (k = 0; n < num1; k++, n++) {
		...
		if (gs2_p->notelist[k].accidental != noteptrs[n].top_p->accidental &&
		    gs2_p->notelist[k].accidental != '\0' &&
		    noteptrs[n].top_p->accidental != '\0')
			return (NO);
	...
	for (k = 0; n < num1; k++, n++) {
		...
		if (gs1_p->notelist[n].accidental == '\0') {
			gs1_p->notelist[n].accidental =
					gs2_p->notelist[k].accidental;
		} else if (gs2_p->notelist[k].accidental == '\0') {
			gs2_p->notelist[k].accidental =
					gs1_p->notelist[n].accidental;
		}
...

{
	...
	for (gs_p = gsnorm_p->prev; gs_p != 0 && gs_p->grpvalue == GV_ZERO;
		...
		/*
		 * Grace groups need accidentals applied to them right away,
		 * since we want to include them in their group boundaries.  We
		 * can do this, since there is always only the one group to
		 * deal with at a time.
		 */
		gs_p->sep_accs = YES;
...

{
	...
	for (found = 0, k = nextaccparen(noteptrs, what, found);  k != -1;
		...
		/* stack acc, if note has one and we're doing them this pass */
		if (note_p->accidental != '\0' && (what & SL_ACC) != 0) {
			stackwhat |= SL_ACC;
		}
...

{
	...
	for (n = previdx + 1; n < lastidx; n++) {
		note_p = GETPTR(n);
		if ((what & SL_ACC) != 0   && note_p->accidental != '\0' ||
...

{
	...
	float grace_rn, grace_rs, grace_re;	/* graces' relative coords */
	...
	for (gidx = 0; gidx < numgrps; gidx++) {
		...
		/*
		 * Set grace_re to be the leftmost main group boundary of any
		 * group that vertically overlaps our group's rightmost grace.
		 */
		grace_re = gwest[gidx];
		grace_rn = g_p[gidx]->prev->c[RN];
		grace_rs = g_p[gidx]->prev->c[RS];
		for (gidx2 = 0; gidx2 < numgrps; gidx2++) {
			if (grace_rn > g_p[gidx2]->c[RS] &&
			    grace_rs < g_p[gidx2]->c[RN] &&
			    g_p[gidx2]->c[RW] < grace_re) {
				grace_re = g_p[gidx2]->c[RW];
			}
		}
...

{
	...
	if (found == 0) {
		for (pidx = MAXHAND - 1; pidx >= 0; pidx--) {
			for (gidx = 0; gidx < numgrps; gidx++) {
				note_p = n_p[gidx][pidx];
				if (note_p != 0 && note_p->accidental != '\0') {
 ...
 *		should be put on.  It sets normwith accordingly.
 */
