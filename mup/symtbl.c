 ...
 * 2006, 2007, 2009, 2010, 2011 by Arkkra Enterprises */
/* All rights reserved */
...
{
	if ( IS_MUSIC_FONT(font) == NO || ch < 0 || ch >= MAX_CHARS_IN_FONT) {
...

{
	...
	/* Do error checking */
	if (code < FIRST_CHAR || code >= FIRST_CHAR + MAX_CHARS_IN_FONT) {
...

{
	...
	if ( IS_MUSIC_FONT(font) == NO || headch < 0 || headch >= MAX_CHARS_IN_FONT) {
